...
`extract_node_features()` 必须从共现矩阵中提取固定维度 F≈16 的统计特征向量，每个特征与数据集点数 N 无关。

#### 场景: 提取完整特征集

- **当** 输入为 N×N 共现矩阵
- **那么** 系统对每个节点计算 16 维特征向量，包含：平均共现度、最大共现度、共现稀疏度、标准差、响应频次、平均响应大小、度中心性、局部聚类系数、特征向量中心性近似、PageRank 近似、共现集中度、自信息
- **而且** 保留特征位 F13-F16 初始为 0，供后续扩展

#### 场景: N 极小的情况

- **当** N < 4
- **那么** 系统返回全零特征矩阵 `[N, 16]` 和空边索引
...
`gnn_range_attack.compute_cooccurrence_matrix()` 必须从采样响应集合计算归一化共现矩阵。此需求本身不变，但调用方新增了特征提取步骤。
...
`_load_model()` 必须支持传入模型对象和文件路径两种模式。Checkpoint 格式从 `input_dim` 变更为 `feature_dim`。
